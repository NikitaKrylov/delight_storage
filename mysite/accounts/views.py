...
def switch_subscription(request, *args, **kwargs):
    ...
    if user.subscriptions.filter(subscription_object__id=kwargs["object_id"]).exists():
        user.subscriptions.filter(
            subscription_object__id=kwargs["object_id"]
        ).delete()  # больше нету
        ctx["has_sub"] = False
    else:
        sub = Subscription(
...
def edit_folder(request, *args, **kwargs):
    form = UserFolderForm(
        request.POST, instance=request.user.folders.get(pk=kwargs["pk"])
    )
    if form.is_valid():
        folder = form.save()
...
class UserInfoView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        posts = Post.objects.filter(author=self.object).exclude(
            status=Post.STATUS.DEFERRED
        )
        context["posts"] = posts
        (
            context["likes_count"],
...
class UserPostList(PostListMixin, PostFilterFormMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["likes_count"] = self.object_list.aggregate(lc=Sum("likes_amount"))[
            "lc"
        ]
        return context
...
def delete_all_notification(request, *args, **kwargs):
    ...
    if notifications.count() > 0:
        messages.success(
            request, "Удалено уведомлений - {}".format(notifications.count())
        )
    else:
...
def read_all_notification(request, *args, **kwargs):
    ...
    if notifications.count() > 0:
        messages.success(
            request, "Уведомлениый прочитано - {}".format(notifications.count())
        )
    else:
        messages.info(request, "Список уведомлений пуст")
...
class SelfUserPostListView(
    LoginRequiredMixin, PostListMixin, PostFilterFormMixin, ListView
):
    model = Post
    login_url = reverse_lazy("login")
    context_object_name = "posts"
    ...
    def get_context_data(self, *args, object_list=None, **kwargs):
        ...
        # views
        context["dates"], context["views_values"] = ChartStatistic(
            Like.objects.filter(post__author=user), "creation_date", start, end
        ).create()
        context["likes_values"] = (
            ChartStatistic(
                UserView.objects.filter(post__author=user), "creation_date", start, end
            )
            .create()
            .values
        )
        context["comments_values"] = (
            ChartStatistic(
                Comment.objects.filter(post__author=user), "pub_date", start, end
            )
            .create()
            .values
        )
        ...
        if form.is_valid():
            ...
            return render(
                ...
                self.template_name,
                {
                    ...
                    "video_formset": VideoFileFormSet(instance=form.instance),
                    "tag_form": CreatePostTagForm(),
                },
            )
...
class PostStatisticView(LoginRequiredMixin, CheckUserConformity, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["lvalues"] = [
            self.object.likes.filter(creation_date__date=date).count() for date in dates
        ]

        context["vlabels"] = fdates
        context["vvalues"] = [
            self.object.views.filter(creation_date__date=date).count() for date in dates
        ]

        context["clabels"] = fdates
        context["cvalues"] = [
            self.object.comments.filter(pub_date__date=date).count() for date in dates
        ]
...
class DashBoardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["views_labels"], context["views_values"] = ChartStatistic(
            UserView.objects.filter(post__author=user), "creation_date", start, end
        ).create()
        context["likes_labels"], context["likes_values"] = ChartStatistic(
            Like.objects.filter(post__author=user), "creation_date", start, end
        ).create()
        context["sub_labels"], context["sub_values"] = ChartStatistic(
            Subscription.objects.filter(subscription_object=user),
            "start_date",
            start,
            end,
        ).create()
